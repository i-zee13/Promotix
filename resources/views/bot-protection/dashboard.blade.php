    ...
    <section class="mx-auto w-full px-[12px] pb-[24px] pt-[28px] sm:px-[18px] xl:px-[19px] xl:pt-[68px]">
        ...
        <div class="bp-adv-page-head mb-[14px]">
            ...
            <style>
                ...
                @media (min-width: 1100px) {
                    ...
                    .figma-filter-bar--bp-dash {
                        ...
                        max-width: min(100%, calc(100vw - 320px)) !important;
                ...
                .figma-filter-bar--bp-dash .bp-dash-f-actions {
                    ...
                    border-top: 0;
                    margin-left: 0 !important; /* no empty gap before calendar */
                }
                ...
                /* Tablet / mid: keep one row, allow gentle scroll, no dead space */
            ...
            <div class="figma-filter-bar figma-filter-bar--overview figma-filter-bar--bp-dash ov-filter-bar ml-auto flex min-h-[44px] w-full max-w-full flex-nowrap overflow-x-auto overflow-y-visible rounded-[10px] border border-white/25 bg-[#d9d9d9] text-[10px] text-black shadow-[0_2px_10px_rgba(0,0,0,.35)]">
                ...
                <label class="bp-dash-f-account relative flex flex-col justify-center border-r border-black/20 px-[7px] py-[5px]" @click.outside="filterMenus.account = false">
                    ...
                    <div x-show="filterMenus.account" x-cloak class="paid-advanced-campaign-menu promotix-slim-scroll !left-[7px] !right-auto !min-w-[240px] !z-[80]">
                        ...
                        <template x-for="a in accountOptions" :key="'bp-acc-' + a.id">
...
function botProtectionFigma(config = {}) {
    ...
    return {
        ...
        funnelStepIcon(key) {
            const k = String(key || '').toLowerCase();
            const wrap = (d) => `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${d}</svg>`;
            if (k.includes('view') || k === 'views') {
                return wrap('<path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12z"/><circle cx="12" cy="12" r="3"/>');
            }
            if (k.includes('cart')) {
                return wrap('<circle cx="9" cy="20" r="1"/><circle cx="18" cy="20" r="1"/><path d="M3 4h2l2.4 11.2a2 2 0 002 1.6h7.4a2 2 0 001.9-1.5L21 8H7"/>');
            }
            if (k.includes('checkout')) {
                return wrap('<rect x="3" y="6" width="18" height="13" rx="2"/><path d="M3 10h18M8 6V5a2 2 0 012-2h4a2 2 0 012 2v1"/>');
            }
            if (k.includes('purchase')) {
                return wrap('<path d="M12 2l3 7h7l-5.5 4.2L18.5 21 12 16.8 5.5 21l1.9-7.8L2 9h7z"/>');
            }
            if (k.includes('form')) {
        ...
        paintGeoMap() {
            this.$nextTick(() => {
                ...
                rows.forEach((r) => {
                    ...
                    // Country groups (<g id="fr">) need child paths painted too
                    el.querySelectorAll('.pa-geo__land, path').forEach((path) => path.classList.add(cls));
                });
            });
        },
        pageDevices() {
