...
<div class="brand-page-bg analytics-skin min-h-[calc(100vh-49px)]" x-data="botProtectionAdvancedFigma({ analyticsMode: @json($analyticsMode ?? false), enabledAdPlatforms: @json($enabledAdPlatforms ?? ['meta' => false, 'microsoft' => false]) })" x-init="init()">
    <section class="mx-auto w-full min-w-0 px-[12px] pb-[28px] pt-[28px] sm:px-[18px] xl:px-[19px] xl:pt-[68px]">
        ...
        <style>
            ...
            html.light-mode .bp-adv-country-row__track { background: rgba(255, 102, 0, 0.1); }
            ...
            <div class="figma-filter-bar figma-filter-bar--overview figma-filter-bar--bp-adv ov-filter-bar ml-auto flex min-h-[54px] w-fit max-w-full flex-nowrap overflow-visible rounded-[10px] border border-white/25 bg-[#d9d9d9] text-[10px] text-black shadow-[0_0_0_rgba(255,255,255,.25)]">
                <label class="bp-adv-f-search relative flex shrink-0 flex-col justify-center border-r border-black/20 px-[7px] py-[6px]">
                    ...
                    <div class="figma-filter-path-wrap">
                        ...
                        <input x-model="filters.ip" @input="scheduleReload(true)" :placeholder="analyticsMode ? 'Search Device ID, session, IP…' : 'Search pages, keywords…'" class="figma-filter-control h-[23px] w-full rounded-[3px] border-0 bg-[#101010] py-0 pl-[22px] pr-[8px] text-[10px] text-[#8c8787] placeholder:text-[#8c8787] focus:ring-0">
                    </div>
                </label>
            ...
            <div class="flex flex-wrap items-center justify-between gap-[10px] overflow-visible rounded-t-[12px] bg-[#FF6600] px-[16px] py-[12px]">
                ...
                <div class="flex flex-1 flex-wrap items-center justify-end gap-[10px]">
                    ...
                    <label class="relative flex h-[28px] min-w-[200px] max-w-[280px] flex-1 items-center rounded-[6px] bg-white px-[10px]">
                        ...
                        <input type="search" :placeholder="analyticsMode ? 'Search Device ID / session / IP' : 'Search for IP Address'" x-model="filters.ip" @input="scheduleReload(true)" class="w-full border-0 bg-transparent text-[11px] text-[#121212] placeholder:text-[#8c8787] focus:ring-0">
                    </label>
...
function botProtectionAdvancedFigma(config = {}) {
    ...
    return {
        ...
        domainOptions: @js($domains->map(fn ($d) => ['id' => (string) $d->id, 'label' => $d->hostname])->values()->all()),
        accountOptions: @js(collect($googleAdsAccounts ?? [])->map(function ($account) {
            if (is_array($account)) {
                $accId = (string) ($account['id'] ?? '');
                $accLabel = (string) ($account['label'] ?? 'Account');
                $accCurrency = (string) ($account['currency_code'] ?? '');
            } else {
                $accId = (string) $account->id;
        ...
        trafficFilterLabel() {
            ...
            return hit ? hit.label : (this.analyticsMode ? 'All Sources' : 'Google Ads');
        },
        accountFilterLabel() {
        ...
        pickTrafficFilter(value) {
            this.filters.traffic_source = value;
