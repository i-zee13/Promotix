...
<div class="pi-spec-modal-root" style="position:fixed;inset:0;z-index:2147483000;display:none;" x-show="trackingTemplateModal.open" x-cloak role="dialog" aria-modal="true" @click.self="closeTrackingTemplateModal()" @keydown.escape.window="if (trackingTemplateModal.open) closeTrackingTemplateModal()">
    ...
    <div class="pi-spec-modal-panel relative z-[1] flex w-full max-w-[980px] flex-col overflow-hidden rounded-[12px] border border-white/20 bg-[#121212] text-white shadow-2xl" @click.stop>
        ...
        <div class="pi-spec-modal-body grid gap-0 lg:grid-cols-[minmax(0,1.3fr)_minmax(240px,0.7fr)]">
            <div class="space-y-[12px] border-b border-white/10 px-[18px] py-[16px] lg:border-b-0 lg:border-r">
                ...
                <label class="flex items-center justify-between gap-[8px] text-[12px]"><span>Preserve auto-tagging</span><input type="checkbox" x-model="trackingTemplateModal.preserveGclid" class="accent-[var(--brand-primary)]"></label>
                <label class="flex items-center justify-between gap-[8px] text-[12px]"><span>Preserve existing parameters</span><input type="checkbox" x-model="trackingTemplateModal.preserveParams" class="accent-[var(--brand-primary)]"></label>
            </div>
            <aside class="space-y-[10px] bg-[#161616] px-[16px] py-[14px] text-[12px]">
                <p class="text-[11px] font-semibold uppercase text-white/50">Validation</p>
                ...
                <p class="rounded-[8px] border border-[var(--brand-primary)]/35 bg-[var(--brand-primary)]/10 px-[10px] py-[8px] text-[11px] text-[#ffd0b0]">
                    Not verified — Apply disabled. OAuth/API connection is not third-party click-tracker certification.
                </p>
            </aside>
        </div>
        <footer class="flex shrink-0 flex-wrap items-center justify-end gap-[8px] border-t border-white/15 px-[22px] py-[14px]">
            ...
            <button type="button" class="rounded-[6px] border border-white/30 px-[16px] py-[8px] text-[13px]" @click="showMenuToast('Tracking template draft saved locally.', 'success')">Save draft</button>
            <button type="button" class="rounded-[6px] bg-[var(--brand-primary)] px-[18px] py-[8px] text-[13px] font-semibold text-white" @click="showMenuToast('Template test queued (safe mode).', 'success')">Test template</button>
            <button type="button" class="rounded-[6px] border border-white/20 px-[18px] py-[8px] text-[13px] text-white/40" disabled>Apply</button>
        </footer>
    </div>
</div>
</template>
