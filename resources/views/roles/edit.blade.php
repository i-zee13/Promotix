...
<div class="min-h-[calc(100vh-49px)] px-[12px] pb-[28px] pt-[20px] sm:px-[18px]" style="background:var(--brand-background,#0d0d0d);">
    <div class="mb-[16px] flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-[28px] font-semibold text-white">Edit {{ $role->name }}</h1>
            <p class="mt-1 text-[13px] text-white/60">Update role details and permissions.</p>
        </div>
    ...
    <form method="POST" action="{{ route('roles.update', $role) }}" class="mx-auto max-w-3xl space-y-5 rounded-[12px] border border-white/15 p-[18px] sm:p-[22px]" style="background:color-mix(in srgb, var(--brand-primary,#6400B2) 22%, #101010);" x-data="{ filter: '' }">
        @csrf
        @method('PATCH')
        <div>
            ...
            <input id="name" name="name" type="text" value="{{ old('name', $role->name) }}" required
                class="w-full rounded-[8px] border border-white/25 bg-black/30 px-[12px] py-[10px] text-[13px] text-white focus:border-white focus:outline-none">
        ...
        <div>
            ...
            <input id="slug" name="slug" type="text" value="{{ old('slug', $role->slug) }}" required @disabled($role->slug === 'super-admin')
                class="w-full rounded-[8px] border border-white/25 bg-black/30 px-[12px] py-[10px] text-[13px] text-white focus:border-white focus:outline-none disabled:opacity-50">
            @error('slug')<p class="mt-1 text-[12px] text-rose-300">{{ $message }}</p>@enderror
        </div>
        <div>
            <label for="description" class="mb-1.5 block text-[12px] font-semibold uppercase tracking-wide text-white/70">Description</label>
            <textarea id="description" name="description" rows="2"
                class="w-full rounded-[8px] border border-white/25 bg-black/30 px-[12px] py-[10px] text-[13px] text-white focus:border-white focus:outline-none">{{ old('description', $role->description) }}</textarea>
        </div>
        ...
        <div class="flex flex-wrap gap-3 pt-2">
            <button type="submit" class="rounded-[8px] px-[16px] py-[10px] text-[13px] font-semibold text-white" style="background:var(--brand-primary,#6400B2);">Save changes</button>
