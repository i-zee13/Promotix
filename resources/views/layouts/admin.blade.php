...
@php
    ...
    $navGroups = [
        ...
        'ANALYTICS' => [
            ...
            ['label' => 'Visitor Journey', 'route' => 'analytics.journeys', 'icon' => 'repeat', 'permission' => 'bot-protection'],
        ],
        'SITE MANAGEMENT' => [
                ...
                <div class="figma-header-userchip flex h-[36px] max-w-[60vw] items-center overflow-hidden rounded-[4px] border border-[#6400B2] bg-[#0D0D0D] text-[11px] leading-none text-white sm:max-w-none">
                    @if ($user?->canInviteTeamMembers())
                        <button
                            ...
                            class="figma-header-avatar flex h-full w-[34px] shrink-0 items-center justify-center overflow-hidden bg-white/10 hover:bg-white/20"
                            title="Invite teammate"
                            aria-label="Invite teammate"
                        >
                            @include('partials.user-avatar', ['avatarUser' => $user, 'avatarTextClass' => 'text-[11px] font-semibold leading-none text-white/90'])
                        </button>
                    @else
                        <span class="figma-header-avatar flex h-full w-[34px] shrink-0 items-center justify-center overflow-hidden bg-white/10">
                            @include('partials.user-avatar', ['avatarUser' => $user, 'avatarTextClass' => 'text-[11px] font-semibold leading-none text-white/90'])
                        </span>
                    @endif
                    <button type="button" @click="userMenuOpen = ! userMenuOpen" class="inline-flex h-full items-center truncate px-[10px] pl-[4px] text-left text-[11px] font-medium leading-none sm:pr-[12px]">{{ $user?->name ?: ($user?->email ?? 'User') }}</button>
                </div>
