    ...
    <div class="onboarding-checkout__inner">
        ...
        >
            ...
            <div class="onboarding-checkout__pay">
                ...
                @if (! empty($stripeEnabled) && $verifyCents > 0)
...
<script>
    (function () {
        ...
        if (form.dataset.stripeEnabled !== '1' || !window.Stripe) return;
        ...
        form.addEventListener('submit', async function (event) {
            ...
            const expMonth = (expMonthHidden?.value || '').trim();
            const expYear = (expYearHidden?.value || '').trim();
            ...
            if (result.error || !result.paymentMethod?.id) {
                ...
                alert(result.error?.message || 'Card verification failed. Please check your card details.');
                return;
            }
