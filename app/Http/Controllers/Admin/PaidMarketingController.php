...
class PaidMarketingController extends Controller
{
     ...
     */
    private function trafficControlFieldsForDetailedVisit(
        PaidMarketingVisit $visit,
        ?object $recording,
        ?\App\Models\User $user,
        mixed $firstClickAt,
        mixed $lastClickAt,
        ?object $behaviorCounts,
        int $invalidClicks,
        mixed $firstClick,
        array $intel,
    ): array {
    ...
    private function computeDetailedStatsFromArrays(Collection $rows): array
    {
        ...
        foreach ($rows as $visit) {
            ...
            $valid = (int) ($visit['valid_clicks'] ?? max(0, $visits - $invalid));
            $isInvalid = $invalid > 0
            ...
            if ($weight > 0) {
                // Priority: Repeat → VPN → DC → Geo → Automated → Suspicious
                if ($isRepeat) {
                ...
                $threatBuckets['valid']['count'] += $valid;
            }
            ...
            if ($level === 'critical' || $level === 'crit') {
                ...
                $group = strtolower(trim((string) ($visit['threat_group'] ?? '')));
                $level = in_array($group, ['data_center', 'datacenter', 'vpn', 'malicious'], true) ? 'high' : ($isInvalid ? 'medium' : 'low');
            }
        ...
        if ($threatGradientStops === []) {
            $threatGradientStops[] = 'rgba(255,102,0,0.25) 0% 100%';
        }
        ...
        $settings = DomainDetectionSetting::updateOrCreate(
            ...
            [
                ...
                'detection_thresholds' => [
                    ...
                    'hourly_valid_click_limit' => (int) ($data['hourly_valid_click_limit'] ?? 2),
                    'daily_valid_click_limit' => (int) ($data['daily_valid_click_limit'] ?? 3),
